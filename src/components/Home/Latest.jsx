...
import { FaCheck, FaEthereum, FaHeart } from "react-icons/fa"
                    ...
                    products.length > 0 ? products.map((product, i) => (
                        <div key={product.id} className="my-2 border border-gray-200 shadow-lg rounded bg-white relative">
                            ...
                            <div className="absolute right-4 top-2">
                                <FaHeart onClick={() => dispatch(addWishList(`${product.id}`))} className={wishId.includes(product.id) ? "text-3xl text-yellow-300 hover:cursor-pointer hover:text-yellow-400": "text-3xl text-white hover:cursor-pointer hover:text-gray-100"} />
                            </div>
                        </div>
                    ))
                    : null
